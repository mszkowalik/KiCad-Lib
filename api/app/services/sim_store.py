...
from .simcompose import COMPOSED_KIND, WRAPPER_PREFIX, compose, wrapper_name
...
def propose_sim_model_version(
    ...
    # KiCad runs ngspice in `pslta` compatibility mode, where `$` is NOT a
    # comment: numparam feeds the rest of the line to the expression parser
    # and the model fails to load. `sigma_hss` shipped with seven of them
    # and failed in exactly that way (2026-09-07). `;` is the comment.
    for lineno, line in enumerate(source_text.splitlines(), 1):
        if "$" in line and not line.lstrip().startswith("*"):
            return {"error": f"line {lineno} carries a `$` — KiCad's compatibility mode "
                             "does not treat it as a comment; use `;`"}

    model = db.query(M.SimModel).filter_by(name=name).first()
    is_new = model is None
    # The `sigma_sym_` namespace is GENERATED by the composer, which is the
    # only caller that passes kind=composed. A hand edit of a wrapper would
    # publish, stay `composed`, and be withheld by the mirror as behind its
    # design; a hand-written model under the prefix would collide with the
    # next composition. Both were accepted before (2026-09-07).
    if kind != COMPOSED_KIND:
        if model is not None and model.kind == COMPOSED_KIND:
            return {"error": f"{name!r} is a composed wrapper — edit its composition on the "
                             "symbol, not its text"}
        if name.startswith(WRAPPER_PREFIX):
            return {"error": f"the {WRAPPER_PREFIX}* namespace belongs to composed wrappers; "
                             "name a hand-written model differently"}
    if is_new:
    ...
    if cur is not None and cur.source_text == source_text:
        # A kind-only change is still a change: without this commit the
        # session closed and the kind quietly reverted.
        db.commit()
