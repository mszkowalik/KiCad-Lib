...
def acting_name() -> str:
    ...
    Always the SIGNED-IN person behind the request — the session cookie or the
    personal token. No endpoint accepts a name from the client (decision
    0050): a typed name is a claim anyone could make. With nobody signed in
    (dev, `auth_enabled` off) it is the placeholder `"user"`. Reads the request
    context that `authgate` binds, so it needs no `Request`.
    """
