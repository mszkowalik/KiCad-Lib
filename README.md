...
**The platform is the library now.** Postgres is the source of truth; the
original YAML pipeline (`Sources/*.yaml`, `kicad_lib/`, `main.py`) that seeded
it is retired and preserved — with its full history and final generated state —
on the **`archive/yaml-library`** branch.
...
The database was seeded by a one-time YAML import; the platform is
self-contained now. To re-run a full import (**wipes the database**), check
out the `archive/yaml-library` branch in this working tree (it holds
`Sources/*.yaml`, base symbols, footprints, 3D models — mounted read-only at
`/repo`), then web UI → **Import** → type `IMPORT` → run.
...
`ANTHROPIC_API_KEY` in `.env`** (compose passes it through; dev mode:
