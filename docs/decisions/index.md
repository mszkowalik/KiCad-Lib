...
| [0052](0052-a-jlc-lot-costs-what-jlc-settled.md) | A JLC parts lot costs what JLC SETTLED — the sub-order's `settlePaidMoney`, after refunds and supplements, never the advance; a parts refresh re-prices the lot AND the draws bound to it in one journalled batch, voids refunded cancellations, and refuses to move a closed batch | Backend, production economics |
| [0051](0051-no-endpoint-accepts-a-name-from-the-client.md) | No endpoint accepts a name from the client — `?actor=` and the body `author`/`created_by`/… fields are REMOVED, not ignored; `acting_name()` takes no argument, and a test fails if a route grows one again (supersedes item 6 of [0050](0050-every-change-names-the-person-who-made-it.md)) | Backend, access control, audit |
